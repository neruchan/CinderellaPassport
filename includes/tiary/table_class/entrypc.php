...
class entrypc extends ipfDB{
	...
	//トップページ投稿公開全体数取得
	function selectContributeCntPC($category='',$category1='',$keyword=""){
		$sqlnew='';
		$sqllike='';
		$sqlcategory='';
		$sqllikes = "";
...
// 			$sqlnew=' ORDER BY sc.sc_addtime DESC ';
		...
		$keywordsql $sqlcategory  $sqlnew $sqllike ";
		//  		print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
	...
	//PCトップページ投稿公開全体情報取得
	function selectContributeAllPC($category='',$category1='',$num=20,$page=0,$keyword=""){
		$sqlnew='';
		$sqllike='';
		$sqlcategory='';
		$sqllikes = "";
...
// 		print $sql;
	...
	//トップページ投稿公開全体数取得
	function selectContributeCntPC1($category='',$category1='',$keyword=""){
		$sqlnew='';
		$sqllike='';
		$sqlcategory='';
		$sqllikes = "";
		...
		//PCトップページ投稿公開全体情報取得
		function selectContributeAllPC1($category='',$category1='',$num=20,$page=0,$keyword=""){
		$sqlnew='';
			$sqllike='';
			$sqlcategory='';
			$sqllikes = "";
	...
	//店舗全体数取得
	function selectShopCntPC($category='',$category1='',$keyword="",$city_data_str=""){
		$sqlnew='';
		$sqllike='';
		$sqlcategory='';
		$sqllikes = "";
...
// 		print $sql;
	...
	function selectContribute($id,$userid=""){
		...
		$sql = "SELECT sc.*,ui.userid,ui.userpic,ui.username,ui.nickname,ui.model_id FROM startyfreecontribute sc JOIN users_info ui ON ui.userid=sc.sc_userid WHERE id=$id  $userid AND sc.sc_delete_flag=0 ";
		//print $sql;
		$data = $this->query($sql);
		return $data;
	}
...
// 		 		print $sql;
	...
	function selectBookmarkTrue($eid,$uid,$flag=0){
		if(!$eid)
		return array();
		$sql = "SELECT count(*)cnt FROM startyfreebookmark WHERE sb_sc_id=$eid AND sb_userid=$uid ";
		//AND sb_flag = $flag
// 		print $sql;
		$data = $this->query($sql);
	...
	//ブックマーク一覧
	function selectContributeBookmarkMyCnt($uid){
		if(!$uid)
		return array();
// 		$sql = "SELECT
// 					count(*)cnt
...
// 				";
	...
	//ブックマーク一覧
	function selectContributeBookmarkMy($uid,$num=6,$page=0){
		if(!$uid)
		return array();
// 		$sql = "SELECT
// 				s.id,
...
// 				print $sql;
	...
	//お気に入りユーザー数
	function selectDealUserMyCnt($userid){
		if(!$userid)
		return array();
		$sql = "SELECT count(*)cnt FROM startyfreeplayer sp JOIN users_info ui ON ui.userid = sp.sp_userfriendid WHERE sp.sp_userid = $userid ";
// 				print $sql;
		$data = $this->query($sql);
	...
	function selectDealUserMy($userid,$num=6,$page=0){
		if(!$userid)
		return array();
		$sql = "SELECT ui.userid,ui.username,ui.nickname,ui.userpic,sp.sp_userfriendid FROM startyfreeplayer sp JOIN users_info ui ON ui.userid = sp.sp_userfriendid WHERE sp.sp_userid = $userid  LIMIT $num OFFSET ".($page * $num)." ";
// 		print $sql;
		$data = $this->query($sql);
...
// 				print $sql;
	...
	//ユーザー投稿公開全体情報取得
	function selectEntryUseCnt($userid="",$usertype=""){
		if(!$userid)
		return array();
		$sql = "SELECT
						count(*)cnt
	...
	//ユーザー投稿公開全体情報取得
	function selectEntryUse($userid="",$num=20,$page=0,$usertype=""){
		if(!$userid)
		return array();
		$sql = "SELECT
					sc.*, ui.userid,
...
// 		print $sql;
	...
	//ユーザー投稿公開全体情報取得
	function selectMyEntry($userid="",$num=20,$last_id,$usertype=""){
		if(!$userid)
		return array();
		$sql = "SELECT
						sc.*, ui.userid,
...
// 				print $sql;
	...
	//ユーザー可愛いリスト情報取得
	function selectEntrykawaiiCnt($userid="",$usertype=""){
		if(!$userid)
		return array();
		$sql = "SELECT
							count(*)cnt
	...
	//ユーザー可愛いリスト情報取得
	function selectEntrykawaii($userid="",$num=20,$page=0,$usertype=""){
		if(!$userid)
		return array();
		$sql = "SELECT
						sc.*, ui.userid,
...
// 			print $sql;
	...
	function selectReadNum($uid){
		if(!$uid)
		return array();
		$sql = "SELECT id,sc_userid,sc_img,sc_title FROM startyfreecontribute WHERE sc_userid = $uid AND sc_read_flag = 1 AND sc_delete_flag=0 ";
		// 		print $sql;
		$data = $this->query($sql);
		return $data;
	}
...
// 		return array();
	...
	//投稿店舗数情報取得
	function selectShopContributeAll($spid="",$num=20,$page=0){
		if(!$spid)
		return array();
		$sql = "SELECT
							sc.*, ui.userid,
...
// 				print $sql;
	...
	//メール送信チェック
	function mailSettingCheckMessage($uid,$type){
		if(!$uid)
		return array();
		$sql = "SELECT u.email FROM startyfreemail sm JOIN users u ON u.id=sm.sma_userid WHERE sm.$type = 1 AND sm.sma_userid = $uid ";
		// 				print $sql;
		$data = $this->query($sql);
		return $data;
	}
	...
	//メール送信可愛いチェック
	function mailSettingCheckKwaii($eid){
		if(!$eid)
		return array();
		$sql = "SELECT
						u.email,
					...
					FROM
						startyfreecontribute sc
					JOIN startyfreemail sm ON sm.sma_userid = sc.sc_userid
					JOIN users u ON u.id = sc.sc_userid
					WHERE
						sc.id = $eid
					AND sm.sma_charm_flag = 1";
		// 				print $sql;
		$data = $this->query($sql);
		return $data;
	}
	...
	//お気に入りユーザー新着投稿メール通知
	function mailSettingGetUID($uid){
		if(!$uid)
		return array();
		$sql = "SELECT
						u.email,
					...
					FROM
						startyfreeplayer sp
					JOIN startyfreemail sm ON sm.sma_userid = sp.sp_userid
					JOIN users u ON u.id = sm.sma_userid
					WHERE
						sp.sp_userfriendid = $uid
					AND sm.sma_user_flag = 1 ";
		// 				print $sql;
		$data = $this->query($sql);
		return $data;
	}
	...
	function selectOfficialCntPC(){
		$sql = "SELECT
								...
								JOIN users_info ui ON ui.userid = sc.sc_userid
								WHERE
								sc.sc_delete_flag = 0 AND sc.sc_secret_flag = 0 AND ui.model_id<>'' ";
		//  		print $sql;
		$data = $this->query($sql);
		return $data[0]["cnt"];
	}
	...
	//PCモデル写真全体情報取得
	function selectOfficialAllPC($num=6,$page=0){
		$sql = "SELECT
						sc.*, ui.userid,
						ui.userpic,
						ui.username,
	...
	//店舗コメント数取得
	function selectShopCommentNum($eid=""){
		if(!$eid)
		return array();
		$sql = "SELECT
						count(*)cnt
	...
	//店舗コメント情報取得
	function selectShopComment($sid="",$num=20){
		if(!$sid)
		return array();
		$sql = "SELECT
					i.id,
	...
	//店舗コメント情報取得
	function selectShopCommentAdd($sid="",$num=20,$last_id){
		if(!$sid)
		return array();
		$sql = "SELECT
					i.id,
	...
	//PCモデル写真全体情報取得
	function selectModelAllPC($num=8,$page=0){
		$sql = "SELECT
						ui.username,
						ui.userid,
						ui.userpic,
...
// 		$sql = "UPDATE banner_advertising SET click_num =click_num+1  WHERE id = $bid ";
	...
	function selectAllDeviceId($type=0){
...
// 	}
    ...
    function selectNewsForApps($userid){
    	if(!$userid)
            return array();
		...
		(
			(
				...
				ui.userpic as userpic,
				sd.sd_addtime as addtime,
				'' as entry_id,
				sd.sd_detailed as content_message,
				'1' as flag
				FROM
				startyfreedirect sd
				JOIN users_info ui ON ui.userid = sd.sd_userid
				WHERE
					sd_receive_userid = $userid
				AND sd_read_flag = 0
			)
			UNION
			(
				...
				ui.nickname as nickname,
				ui.userpic as userpic,
				c.c_addtime as addtime,
				c.c_sc_id as entry_id,
				'' as content_message,
				'2' as flag
				FROM 
				charmuser c 
				JOIN users_info ui ON ui.userid = c.c_userid 
				WHERE c.c_sc_id IN 
				(
					SELECT id FROM startyfreecontribute WHERE sc_userid = $userid AND sc_read_flag = 1 AND sc_delete_flag=0
				)
				ORDER BY addtime DESC LIMIT 1

			)
		)a ORDER BY addtime DESC
        ";
