<?php
class sp_register extends ipfDB{
    ...
    function selectUsersname($usersname){
        $sql = "SELECT count(*) as cnt FROM users WHERE username = '$usersname'";
    ...
    function selectUID($uid){
    	if(!$uid)
    	return array();
    	$sql = "SELECT count(*) as cnt FROM users  WHERE id=$uid";
    	$data = $this->query($sql);
    	return $data[0]['cnt'];
    }

    function startyActivate($uid){
    	if(!$uid)
    	return array();
    ...
    function checkUsername($uid){
    	if(!$uid)
    	return array();
    	$sql = "SELECT count(*) as cnt FROM users  WHERE id=$uid AND activated=1";
    	$data = $this->query($sql);
    	return $data[0]['cnt'];
    }
